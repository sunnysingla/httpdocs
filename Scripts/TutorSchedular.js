...
function FillTable(objTables) {
    ...
    MaxColumn = parseInt(window.innerWidth / 380);
...
function drop(event, obj) {
    ...
    if (index != null && index != "null") {
        ...
        if (forstudent_source == 0) {
            ...
            PostDataGeneric("Swipe_Tutor", "Schedule.aspx", StringData, true);
            return false;
        }
    }
    ...
    if (index != null && index != "null") {
        if (msid1 != null && msid1 != "null" && msid2 != null && msid2 != "null") {
            ...
            data.ID2 = msid2;
            var StringData = JSON.stringify(data);
            PostDataGeneric("Swipe_Student", "Schedule.aspx", StringData, true);
            return false;
        }
        else {
            var data = {};
            ...
            var StringData = JSON.stringify(data);
            PostDataGeneric("Swipe_Student_BlankMSID", "Schedule.aspx", StringData, true);
    ...
    if (event.dataTransfer.getData("forstudent") == obj.getAttribute('forstudent')) {
        if (obj.getAttribute('forstudent') == 1) {
            ...
            data.Tablenumber = $($(obj).parent().parent().parent().parent()).attr('tableid');
            var StringData = JSON.stringify(data);
            PostDataGeneric("MasterSchedule_ADD_Update", "Schedule.aspx", StringData, true);
        }
        else {
            var data = {};
            data.TutorID = ID;
            ...
            data.Tablenumber = $($(obj).parent().parent().parent().parent()).attr('tableid');
            var StringData = JSON.stringify(data);
            PostDataGeneric("MasterSchedule_ADD_Update_Tutor", "Schedule.aspx", StringData, true);
        }
    }
...
function MasterScheduling() {
    ...
    $.ajax({
        type: "POST",
        url: "Schedule.aspx/MasterScheduling",
        ...
        success: function (response) {
            ...
            var dvTutor = JSON.parse(response.d[1]);
            var dvStudent = JSON.parse(response.d[2]);
            FillTable(objTables);
...
function LoadEvents() {
    ...
    //        if (SId3 != undefined)
    //            Ids = Ids + ',' + SId3;
    //        DeleteGeneric('DeleteTble', 'Schedule.aspx', Ids, true);
    //    }
    //});
    ...
    $("input[id*=TableIndex_]").blur(function () {
        ...
        if ($($(this).parent().parent().parent().parent()).attr('tableid') != $(this).val()) {
            ...
            var StringData = JSON.stringify(data);
            PostDataGeneric("UpdateTableNumber", "Schedule.aspx", StringData, false);
    ...
    $("#leftDiv td input[type=text]").on("click", function () {
        ...
        if ($(this).attr('index') == '0')
        {
            ...
            if (tutorId != "0" && tutorId != null && tutorId != "null")
                CheckId(tutorId, "0", this);
        }
        else
        {
            var StudentId = "";
            ...
            StudentId = $(ObjParent).attr("allstudentsids").split("_")[parseInt(Index)-1];
            if (StudentId != "0" && StudentId != null && StudentId != "null")
            {
                CheckId(StudentId, "1", this);
            }
        }
    ...
    $(".close-icon").on("click", function () {
        if (confirm('Please confirm you wish to remove this individual from the session.')) {
            var Id = $(this).attr("MSID");
            if (Id != undefined)
                DeleteGeneric("DeleteStudent", "Schedule.aspx", Id, true);
            else {
                ...
                var StringData = JSON.stringify(data);
                PostDataGeneric("DeleteTutor", "Schedule.aspx", StringData, true);
            }
        }
        return false;
        var Data = {};
    ...
    $('input[placeholder*=' + placeholder + ']').on("keydown", function (event) {
        ...
        select: function (event, ui) {
            ...
            if (data.MasterScheduleId != "null" && data.MasterScheduleId != null)
                ...
                PostDataGeneric_NoMessage("UpdateSubject", "Schedule.aspx", StringData, false);
            else {
