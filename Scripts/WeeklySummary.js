...
function WeeklySummary(Date) {
    ...
    $.ajax({
        type: "POST",
        url: "ScheduleSummary.aspx/WeeklySummary",
        data: '{Date:"' + Date + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
...
function drop(event, obj) {
    ...
    if (parseInt(obj.getAttribute("tablenumber")).toString().trim() == "NaN" || parseInt(obj.getAttribute("tablenumber")) == NaN) {
        ...
        PostDataGeneric("Swipe_Student_BlankMSID_Summary", "WeeklySchedule.aspx", StringData, true);
    ...
    if (event.dataTransfer.getData("forstudent") == obj.getAttribute('forstudent')) {
        if (obj.getAttribute('forstudent') == 1) {
            ...
            console.log(StringData);
            PostDataGeneric("MasterSchedule_ADD_Update_All", "WeeklySchedule.aspx", StringData, true);
        }
        else {
            var data = {};
            data.TutorID = ID;
            ...
            PostDataGeneric("MasterSchedule_ADD_Update_Tutor_All", "WeeklySchedule.aspx", StringData, true);
        }
    }
...
function AutoComplete_Tutor(placeholder, src, column) {
    ...
    $('input[forstudent=0]').on("keydown", function (event) {
        ...
        select: function (event, ui) {
            ...
            PostDataGeneric("MasterSchedule_ADD_Update_TutorDayTime", "WeeklySchedule.aspx", StringData, true);
...
function AutoComplete_Student(placeholder, src, column) {
    ...
    $('input[forstudent=1]').on("keydown", function (event) {
        ...
        select: function (event, ui) {
            ...
            var StringData = JSON.stringify(data);
            if (data.TutorID != "0")
                PostDataGeneric("MasterSchedule_ADD_UpdateDayTime", "WeeklySchedule.aspx", StringData, true);
            else {
