...
+function ($) {
	...
	DCalendar.prototype = {
        ...
        selectDate: function () {
            ...
            var newDate = that.formatDate(that.options.format);
            var newDateStr = newDate;
            if (that.options.format.indexOf('XYZ') >= 0)
            {
                var dt = newDate.replace("XYZ ", "");
                var dtDay = dt.split('-')[0];
                var dtMonth = dt.split('-')[1];
                var dtyear = dt.split('-')[2];
                var date = new Date();
                date.setMonth(dtMonth - 1);
                date.setDate(dtDay);
                date.setYear(dtyear);
                newDate = date.toString().substring(0, 15);
                var newDatearry = newDate.split(' ');
                newDateStr = newDatearry[0] + ", " + newDatearry[1] + " " + newDatearry[2] + ", " + newDatearry[3];
 
                setTimeout(function () { getTimes1(newDatearry[0]); }, 100);
                setTimeout(function () { WeeklySummary(dtyear + '-' + (dtMonth).toString() + '-' + dtDay); }, 100);
            }
 
            setTimeout(function () { ClearTime(that.elem[0]); }, 100);
            e = $.Event('dateselected', { date: newDateStr });
