...
function imageLinkFormatter_Invoice(cellval, options, rowObject) {
    var link = '';
    if (rowObject.InvoiceGeneratedID != 0) {
        var img = '<span class="ui-icon ' + options.colModel.formatoptions.icon + ' icon">View<span/>';
        var link = '<a onclick="CheckId(this);"  href="javascript:void(0)" class="' +
            options.colModel.formatoptions.link_class + '" rel="' + rowObject.ID_X + '" lavel="10">' + img + '</a>';
    }
    return link;
}
